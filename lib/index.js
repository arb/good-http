...
internals.GoodHttp.prototype.stop = function () {
    ...
    this._sendMessages(this._eventQueue);
};
...
internals.GoodHttp.prototype._report = function (event, eventData) {
    ...
    if (this._eventQueue.length >= this._settings.threshold) {
        var events = this._eventQueue.slice(0);
        this._eventQueue.length = 0;
        this._sendMessages(events);
    }
};


internals.GoodHttp.prototype._sendMessages = function (events) {
    ...
    envelope.events = internals.createEventMap(events);
